...
| Kibana     | Graphical upload via Kibana interface (Machine Learning → Data Visualizer → Import Data) | <ul><li>simple / low code</li><li>GUI, drag 'n drop</li><li>supports various formats (including CSV/TSV, newline delimited JSON)</li><li>allows live preview of the data, including statistics</li><li>mapping can be edited during import</li></ul> | <ul><li>100MB size limit</li><li>only works on ES installations with connected Kibana</li><li>import only possible into a new index, re-index into existing one would be second step</li></ul> | 
